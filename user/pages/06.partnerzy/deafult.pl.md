---
title: 'Nasi partnerzy'
slug: partners
process:
    markdown: true
    twig: true
media_order: 'malmo-widok.jpg,hand-shake.jpg,logo-placeholder.png'
never_cache_twig: true
cache_enable: false
twig_first: false
taxonomy:
    tag:
        - to_menu_footer_informacje
menu: PARTNERZY
---

===
<div id="partnersBox">
    <div class = "partners">
    <!-- IndSol Group -->
        <a href="https://indsolgroup.com/" target="_blank"> <img class="partnerLogo" src="https://indsolgroup.com/wp-content/uploads/2023/02/IndSol-Logo-1.png" alt="placeholder" style="width: 350px; height: auto; border:none"> </a> <br>
        <p style="text-align: center"> IndSol Group </p> <br> <br>
    <!-- AlfaVeta -->
        <a href="https://alfaveta.se/" target="_blank"> <img class="partnerLogo" src="https://alfaveta.se/wp-content/uploads/2020/03/cropped-Alfaveta-i-Oxie-i-skugga-dark-183x61.png" alt="placeholder" style="width: 220px;border:none"> </a> <br>
        <p style="text-align: center"> AlfaVeta </p> <br>
    <!-- ÖRS -->
        <a href="http://www.ors.se/" target="_blank"> <img class="partnerLogo" src="\eu\user\pages\06.partnerzy\orsLogo.png" alt="placeholder" style="width: 200px;border:none"> </a> <br>
        <p style="text-align: center"> ÖRS </p> <br> <br>
    <!-- Lodjuret Hunddagis -->
        <a href="https://lodjuret.com/" target="_blank"> <img class="partnerLogo" src="https://lodjuret.com/____impro/1/onewebmedia/4.gif?etag=%2272c8-533b9c24%22&sourceContentType=image%2Fgif" alt="placeholder" style="width: 200px;border:none; border-radius: 10px; box-shadow: 0px 0px 29px 0px rgba(0,0,0,0.48);"> </a> <br>
        <p style="text-align: center"> Lodjuret Hunddagis </p> <br> <br>
    <!-- ICMS Marine Service AB -->
        <a href="https://www.icmsmarineserviceab.com/" target="_blank"> <img class="partnerLogo" src="https://static.wixstatic.com/media/e2171f_1a6371c380ed475dabc366bdd47de2f6~mv2.png/v1/fill/w_242,h_79,al_c,q_85,usm_0.66_1.00_0.01,enc_auto/Logo.png" alt="placeholder" style="width: 300px;border:none"> </a> <br>
        <p style="text-align: center"> ICMS Marine Service AB </p> <br> <br>
    <!-- Grand Hotel -->
        <a href="https://www.grandilund.se/" target="_blank"> <img class="partnerLogo" src="https://image-tc.galaxy.tf/wisvg-7uzsursjx828boqblz18lx8j4/grand-hotel-header-desktop.svg?width=500" alt="placeholder" style="width: 300px;border:none; border-radius: 10px;box-shadow: 0px 0px 29px 0px rgba(0,0,0,0.48);"> </a> <br>
        <p style="text-align: center"> Grand Hotel </p> <br> <br>
    <!-- Disgusting Food Museum -->
        <a href="https://disgustingfoodmuseum.com/" target="_blank"> <img class="partnerLogo" src="https://disgustingfoodmuseum.com/wp-content/uploads/2018/12/Favicon-very-new.png" alt="placeholder" style="width: 200px;border:none"> </a> <br>
        <p style="text-align: center"> Disgusting Food Museum </p> <br> <br>
        <!-- Malmö Museer -->
        <a href="https://malmo.se/Uppleva-och-gora/Konst-och-museer/Malmo-Museer.html" target="_blank"> <img class="partnerLogo" src="https://southernswedendesigndays.com/sites/default/files/styles/medium/public/2022-11/Malmo%CC%88%20Stad%20logo%402x.png?itok=me8j5KpI" alt="placeholder" style="width: 320px;border:none;"> </a> <br>
        <p style="text-align: center"> Malmö Museer </p> <br> <br>
        <!-- Poolstar -->
        <a href="http://www.poolstar.se/" target="_blank"> <img class="partnerLogo" src="\eu\user\pages\06.partnerzy\poolstarLogo.png" alt="placeholder" style="width: 300px;border:none; border-radius: 10px;box-shadow: 0px 0px 29px 0px rgba(0,0,0,0.48)"> </a> <br>
        <p style="text-align: center"> Poolstar </p> <br> <br>
        <!-- Bazaar Food Market -->
        <a href="https://www.bazaarfoodmarket.se/" target="_blank"> <img class="partnerLogo" src="https://d1r8abvi6c1fdj.cloudfront.net/media?url=s3%3A%2F%2Fsgn-prd-assets%2Fuploads%2F-pLI36S0OmEbEkU2A1NH3&w=300&sign=6CEBpPSBqkCZkRYS0XwhDAA7NWy2beAd4W42zOQwfJQ" alt="placeholder" style="width: 250px;border:none"> </a> <br>
        <p style="text-align: center"> Bazaar Food Market </p> <br> <br>
        <!-- ÖE Terminal -->
        <a href="https://www.ashirson.se/" target="_blank"> <img class="partnerLogo" src="https://www.ashirson.se/wp-content/uploads/2021/09/logo-light-bg-300x84.png" alt="placeholder" style="width: 250px;border:none"> </a> <br>
        <p style="text-align: center"> ÖE Terminal </p> <br> <br>
        <!-- -->
        <a href="" target="_blank"> <img class="partnerLogo" src="\eu\user\pages\06.partnerzy\logo-placeholder.png" alt="placeholder" style="width: 200px;border:none"> </a> <br>
        <p style="text-align: center"> placeholder </p> <br>
    </div>
</div>